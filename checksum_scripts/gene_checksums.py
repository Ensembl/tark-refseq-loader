    ...
    for db in databases:
        ...
        try:
            ...
            for row in results:
                ...
                print('inputs ', loc_checksum, ' ', biotype, ' ', stable_id, ' ', stable_id_version, '\n')
                print('curr bin ', gene_checksum)
                print('curr hex ', binascii.hexlify(gene_checksum).decode('utf-8').upper())

                checksum = checksum_list('utf-8', loc_checksum, biotype, stable_id, stable_id_version)
                print('second calculation.....')
                new_gene_checksum = checksum_list('utf-8', checksum)
                print("\n")

                # convert to binary format
                update_gene_checksum(session, gene_id, binascii.unhexlify(new_gene_checksum))
